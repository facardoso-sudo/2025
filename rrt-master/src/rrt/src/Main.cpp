...
int main(int argc, char **argv) {
    ...
    ros::Publisher chatter_pub = n.advertise<std_msgs::String>("some_chatter", 1000);
    ...
    int numberOfObs = 1;
    double paddingForObjectAvoidance = 0.15;
    double endPointVisibility = 0.25;
    double goalBias = 0.1;
    double isGoalBias = true;
    int smoothingRepeatition = 100;
    int smoothingWindow = 10;
    //ADD robot tb3_1/odom
    //server = new interactive_markers::InteractiveMarkerServer("tb3_1/odom");
    ...
    while (ros::ok()) {
        ...
        if (!isRRTfinished && frame_count > 2) {
            ...
            //Caminho final do caminho mais curto
            ...
            //float inc = 0.3;
            /*
            for(float i =0; i < rrt.plannedPath.size(); i++){
            ...
            */
            ...
            //float error_dist_robo = 99;
            float i = 0;
            //if(i < rrt.plannedPath.size()){

            do{
                //if(i <= rrt.plannedPath.size()){

//robot.pose.orientation.x = robot.pose.orientation.y pose.position.x;
                i = i + 0.03;
                //float error_dist_robo = sqrt(pow(endP.x-i,2)+pow(endP.y-i,2));
                ...
                //}
            }while(i < rrt.plannedPath.size()-1);
...
void initRviz(ros::Publisher marker_pub, RRT &rrt, vector<visualization_msgs::Marker> &obstacles, int frame_count,
    ...
    // Defina dois obstáculos do tipo ponto
...
void generateRandomObstacles(vector<visualization_msgs::Marker> &obstacles, int numberOfObs, RRT &rrt) {
    ...
    for (int i = 0; i < numberOfObs; i++) {
        ...
        obstacle.scale.x = largura;
        obstacle.scale.y = profundidade;
        ...
        obstacle.pose.position.y = rrt.shiftCordinate(yBottom_, yUp_, rrt.getRanddomDouble(yBottom_, yUp_),
                                                      obstacle.scale.y);
        //obstacle.pose.position.x = 0.5;
        //obstacle.pose.position.y = 0.5;
...
/*
    ...
    xleft_ = rrt.xLeft;
    xRight_ = rrt.xLeft + 0.5;
    yBottom_ = rrt.yBottom + 0.3;
    yUp_ = rrt.yUp;
    ...
    xleft_ = rrt.xLeft;
    xRight_ = rrt.xRight - 0.3;
    yBottom_ = rrt.yUp - 0.5;
    yUp_ = rrt.yUp;
